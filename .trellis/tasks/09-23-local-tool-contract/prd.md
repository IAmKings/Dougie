...
- [x] AC1 资源文件 14 行，六个工具各 2 条，两条闲聊期望为空。
- [x] AC2 罐头「全对」报告通过；故意错一条则不通过；`toString()` 不含用例原文。
- [x] AC3 开发者页试跑不把说法或模型原文写入 Logcat；界面没有「已达标」字样，通过与否只看计数报告里的 passed 字段含义（实现用计数句子表达）。消息格式由 `AppLocalToolContractEvalTest` 锁住。2026-09-24 侧载真机：改协议前 `n=14 correct=12`，`app-1` 预测 `time`、`app-2` 为空。把「打开…」的协议例句提前并写明不要调用 `time` 之后复测仍是 `n=14 correct=12 passed=false`，两条打开应用都没有工具调用。按契约停在最小档，不换模型。
- [x] AC4 Play 包不因此加入对话权重或 `:tool:chatllm`。试跑入口若只在侧载调试页能点，Play 上该按钮保持不可用或隐藏，且 `checkChannelLeak` 通过。Play 上按钮在最小档未启用时不可用，并显示「只认已启用的最小档对话模型。」
- [x] AC5 工具注册表名称集合与改动前一致。

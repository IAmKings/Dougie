...
| SMS compose / dial while background | `SmsComposeTool` / `PhoneDialTool` foreground check | `应用不在前台，无法打开应用或链接。` |
| SMS compose / dial no handler | `ACTION_SENDTO` / `ACTION_DIAL` resolve or start fail | `无法打开系统短信或拨号。` |
| SMS / dial number invalid or SMS body empty / over 1000 UTF-16 chars | `PhoneNumber.canonical` / `sms_compose` body | `工具参数无效，已拒绝执行。` |
...
| Tap/swipe with unknown foreground | Accessibility `rootInActiveWindow` and last `TYPE_WINDOW_STATE_CHANGED` package both empty (config must `canRetrieveWindowContent`; do not map this to the bank message) | `无法确认当前应用，已停止点击以免误操作。` |
...
| Debug Rule B pack/engine missing | `AppKokoroRuleBEval.run` cannot install/extract `eval/tts/kokoro` or `SherpaJni` is unavailable | `评测用合成模型尚未就绪` |
| Debug Rule B last missing/empty/corrupt | `AppKokoroRuleBEval.last` cannot parse at least one jsonl row under `filesDir/eval/tts/kokoro-rtf.jsonl` | leave `ruleBMessage` empty (`null`). Do not dump file text. `markNaturalnessOk` with `nScored<5` returns the existing message and does not rewrite. |
...
| Model smoke probe | Settings 测试: ASR short silence `transcribe` no throw; TTS `generatePcm` non-empty, no play; intent `classify("现在几点")` no throw (low confidence OK); chat `generate("你好")` no throw and no Logcat of prompt/completion; embed `AndroidEmbeddingPort.embed("测")` non-null (passage path, no query prefix). Missing layout/JNI uses existing 尚未就绪 / TTS_FAILED / INTENT_* / `CHAT_MODEL_MISSING` / `CHAT_ENGINE_NOT_READY` / `EMBED_MODEL_MISSING` copy | Success: `语音识别测试通过。` / `语音合成测试通过。` / `意图分类测试通过。` / `对话模型测试通过。` / `语义记忆测试通过。` |
| Embed pack missing | `filesDir/models/embed/{model.onnx,vocab.txt}` absent, JNI unavailable, or `embed()` empty/NaN | `离线语义记忆模型尚未就绪。` Recall stays FTS/`LIKE`. |
...
| Isolated Python host tokens | Script contains `urllib` / `socket` / `subprocess` / `ctypes` / `java.` / `android.`, or a path that is absolute / `..` / `://` / outside `filesDir/py_sandbox` | `隔离 Python 不能访问网络或系统接口。` |
| Isolated Python too large | Script > 8KiB or data > 32KiB | `脚本或数据过长，已拒绝执行。` |
| Isolated Python sandbox quota | Write would make `py_sandbox` exceed 32MiB | `沙箱文件过多，已拒绝写入。` |

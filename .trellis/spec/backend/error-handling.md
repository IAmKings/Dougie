...
| Model smoke probe | Settings 测试: ASR short silence `transcribe` no throw; TTS `generatePcm` non-empty, no play; intent `classify("现在几点")` no throw (low confidence OK); chat `generate("你好")` no throw and no Logcat of prompt/completion; embed `embed("测")` non-null (hash bag tokenizer). Missing layout/JNI uses existing 尚未就绪 / TTS_FAILED / INTENT_* / `CHAT_MODEL_MISSING` / `CHAT_ENGINE_NOT_READY` / `EMBED_MODEL_MISSING` copy | Success: `语音识别测试通过。` / `语音合成测试通过。` / `意图分类测试通过。` / `对话模型测试通过。` / `语义记忆测试通过。` |
| Embed pack missing | `filesDir/models/embed/tokenizer.json` absent or hash-bag spec unreadable | `离线语义记忆模型尚未就绪。` Recall stays FTS/`LIKE`. |

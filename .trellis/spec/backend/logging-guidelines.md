...
- Raw microphone PCM / WAV / audio byte arrays, and ASR transcripts in Logcat or `AuditLog`. `AsrEvalReport.toString()` is counts/rates only — never `reference` or `hypothesis`. Chat user-bubble `sourceLabel` is the fixed product string 「语音转写」 (from `speakReply`), never the ASR transcript or utterance.
- TTS utterance text (what `speech_output` speaks) and host reply playback (`finalAnswer` / PCM). `KokoroEvalReport.toString()` is counts/rates only — never utterance `text` or PCM. Debug `ruleBMessage` is that counts string plus relative `eval/tts/kokoro-rtf.jsonl` plus `adb exec-out run-as <packageName> cat files/eval/tts/kokoro-rtf.jsonl` after a successful `run` / `last` / `markNaturalnessOk` (or `KOKORO_EVAL_MODEL_MISSING`); never gold text, jsonl lines, PCM, or 「已达标」. `AppKokoroRuleBEval.last` missing/empty/corrupt jsonl returns null and must not Logcat file text. Chat typewriter `shown` / `streamingText` / `finalAnswer` must not go to Logcat; 播报 still uses full `item.text` without logging it.
- Intent classifier input text, intent labels, slots, or route JSON. `IntentRuleEReport.toString()` is counts/rates only — never utterance `text` or intent labels. Debug `ruleEMessage` is that counts string plus relative `eval/intent/predictions.jsonl` plus `adb exec-out run-as <packageName> cat files/eval/intent/predictions.jsonl` after a successful `run` or `last` (or existing `INTENT_MODEL_MISSING` / `INTENT_ENGINE_NOT_READY`); never gold text, predicted labels, jsonl lines, or 「已达标」. `AppIntentRuleEEval.last` missing/empty/corrupt jsonl returns null — not `INTENT_FAILED` — and must not Logcat file text. `AppIntentRuleEEval` / `runForward` / `last` must not Logcat classify input.

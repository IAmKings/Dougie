...
- TTS utterance text (what `speech_output` speaks) and host reply playback (`finalAnswer` / PCM). `KokoroEvalReport.toString()` is counts/rates only — never utterance `text` or PCM. Chat typewriter `shown` / `streamingText` / `finalAnswer` must not go to Logcat; 播报 still uses full `item.text` without logging it.

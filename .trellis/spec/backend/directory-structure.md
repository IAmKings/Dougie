...
| `:core:runtime` | `LoopEngine`, `TaskManager` (`deleteConversation`), `ConversationPointer`, `ConversationTitles`, `TaskStore` (`deleteByConversation`, `searchCompletedTurns`), `AuditLog`, `EgressGateway.stream`, `ToolCallSanitizer`, `PolicyEngine` | Compose, Android Context, HTTP |
...
| `:data:tasks` (Android) | SQLite `agent_tasks` / `idempotency` / `audit_log`; `deleteByConversation` and `searchCompletedTurns` scan `snapshot_json` in Kotlin (no `conversation_id` column, no DB bump, no SQL `LIKE` on json) | LoopEngine, Compose |

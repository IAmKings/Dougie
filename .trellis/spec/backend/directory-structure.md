...
- Optional `taskStore`: after shortcuts and before the first LLM stream, `priorTurns` is the current `conversationId`'s earlier `COMPLETED` tasks with non-blank `finalAnswer` (failed/blank omitted). `copy()` keeps them; `TaskSnapshotCodec` omits them.

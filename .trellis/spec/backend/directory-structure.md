...
| `:core:runtime` | `LoopEngine`, `TaskManager` (`deleteConversation`), `ConversationPointer`, `ConversationTitles`, `TaskStore` (`deleteByConversation`), `AuditLog`, `EgressGateway.stream`, `ToolCallSanitizer`, `PolicyEngine` | Compose, Android Context, HTTP |
...
| `:data:tasks` (Android) | SQLite `agent_tasks` / `idempotency` / `audit_log`; `deleteByConversation` scans `snapshot_json` then `DELETE … task_id IN (…)` (no `conversation_id` column, no DB bump) | LoopEngine, Compose |
...
- `LoopEngine(..., intentPort: IntentPort? = null, taskStore: TaskStore? = null)` — App wires `AndroidIntentPort` and the task store; CLI/tests omit unless testing prior turns

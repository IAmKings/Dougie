...
| Tool JSON, gates, model install/import | Matching `*ToolTest` / `ModelInstallerTest` / `ModelImporterTest`; `OfficialModelCatalogTest` pins embed as two HF files (~24MB, `isConfigured`); `HoldSpeechRecorder.snapshot()` empty / grows / empty-after-stop in `SpeechInputToolTest` | `./gradlew :core:tool:test` |

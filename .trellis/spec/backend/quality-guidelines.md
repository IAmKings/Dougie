...
| OpenAI SSE / vendor body / shared Chat prompt | `OpenAICompatibleProviderTest`, `ChatPromptAssemblerTest` (Chinese identity, no tool names or OpenBMB/MiniCPM/Qwen in identity, SCREEN metadata only, remote inventory full table, **local inventory seven no-slot plus L2 write/create/open plus screen_match/speech_output; identity/greeting localPrompt omits protocol; identity asks lock Dougie after the user turn; recent conversation window; protocol history has no old-turn tool JSON**), `LocalToolCallParserTest`, `SelectingLlmProviderTest` (`shouldWarmLocalEngine`) | `./gradlew :core:llm:test` |

...
- Committing weights or native blobs: `*.onnx` (except the tiny testdata allowlisted in `.gitignore`), `*.gguf`, `*.litertlm`, `**/jniLibs/`, `third_party/llama.cpp/`, `/eval/` wav dumps or intent `predictions.jsonl`.
...
| Tool JSON, gates, model install/import | Matching `*ToolTest` / `ModelInstallerTest` / `ModelImporterTest`; `OfficialModelCatalogTest` pins embed as two HF files (~24MB, `isConfigured`); `HoldSpeechRecorder.snapshot()` empty / grows / empty-after-stop in `SpeechInputToolTest`; `IntentEvalTest` parser gold plus Rule E JSONL (missing repo-root predictions still pass) | `./gradlew :core:tool:test` |

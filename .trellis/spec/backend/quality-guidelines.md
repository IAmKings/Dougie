...
| Tool JSON, gates, model install/import | Matching `*ToolTest` / `ModelInstallerTest` / `ModelImporterTest`; `OfficialModelCatalogTest` pins embed as two HF files (~24MB, `isConfigured`); `HoldSpeechRecorder.snapshot()` empty / grows / empty-after-stop in `SpeechInputToolTest`; `IntentEvalTest` parser gold plus Rule E JSONL (missing repo-root predictions still pass); `KokoroEvalTest` Rule B JSONL (missing repo-root `eval/tts/kokoro-rtf.jsonl` still pass; sample is not Rule B) | `./gradlew :core:tool:test` |

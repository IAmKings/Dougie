...
| Vendor presets / task model | `LlmVendorsTest`; `AgentTaskTest` (`priorTurns` default empty and survives `copy`; `startedAt`/`endedAt` default null and survive `copy`; `CompletionPath.toUserLabel()`) | `./gradlew :core:model:test` |

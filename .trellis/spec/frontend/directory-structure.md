...
| `:feature:history` | Task History from `TaskStore.listRecent(50)` grouped by `conversationId` (`toHistorySections`, sticky titles 「默认会话」/「对话 n」 or custom prefs name); cards show `durationLabel` / `providerLabel` from `toHistoryItem` (`formatTaskDuration`, `completionPath?.toUserLabel()`); sticky **改名** dialog → `ConversationTitles.setTitle` (no `openConversation`); tap a card opens that window in Chat and focuses `{taskId}:user`; bottom nav **任务** |

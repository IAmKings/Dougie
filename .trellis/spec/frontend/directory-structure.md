...
User-facing screens live in `:feature:*` Android libraries. `:app` hosts `MainActivity`, `Application`, DI, the Quick Settings `TileService`, and the task-progress notifier. Chat↔History 300ms transitions (`SharedTransitionLayout` + `AnimatedContent`) stay in `:app`; feature modules only attach a `sharedBounds` modifier on the History card and Chat user bubble. Features collect `StateFlow` from `:core:runtime`; they do not run the Agent loop on Main.
...
| Module | Owns |
|--------|------|
| `:feature:chat` | Chat Compose UI, `ChatViewModel`, bubble mapping, Confirm Card; terminal `AgentMessage.durationLabel` from `:core:model` `formatTaskDuration` (not a Chat-local bucket); top bar Dougie + current window name (display only) + 灵魂 + 出境; navigate to settings / memory / Permission Center |
| `:feature:settings` | Provider vendor preset + URL/key/model/`max_tokens`, egress consent copy, save to `PreferenceStore`; **可打开的应用** list (`OpenAppsRoute`, prefs key separate from **保存配置**, launcher icons + text filter, no count cap); **模型目录** (`OpenDocumentTree` URI in `modelTreeUri`) + **刷新** 扫描外部树 + 离线模型确认下载（Play: ASR / TTS / 意图 ONNX / **语义记忆**；sideload adds 对话 `.litertlm`；写入外部树再同步 `filesDir`）+ 已安装烟测（`OfflineModelProbe` 由 `:app` 注入，非 AgentTool）；**开发者** row calls `onOpenDebug` only |
...
| `:feature:history` | Task History from `TaskStore.listRecent(50)` (empty search) or `searchHistory` ∪ title hits (non-empty), grouped by `conversationId` (`toHistorySections`, sticky titles 「默认会话」/「对话 n」 or custom prefs name); search field under **任务历史**; cards show `durationLabel` / `providerLabel` / `completedAtLabel` from `toHistoryItem` (`:core:model` `formatTaskDuration`, `completionPath?.toUserLabel()`, `formatCompletedAt(endedAt)`); sticky **改名** dialog → `ConversationTitles.setTitle` (no `openConversation`); non-default sticky **删除** (danger color, confirm dialog) → `TaskManager.deleteConversation` then History `refresh()` plus Activity `listRecent` snapshot for Chat numbering (no `openConversation`; default has no section delete); card **删除** next to **展开** → `TaskManager.deleteTask` then the same History + `listRecent` refresh (no `openConversation`; default cards allowed); tap a card opens that window in Chat and focuses `{taskId}:user`; bottom nav **任务** |
| `:feature:debug` | Developer page: current `AgentTask` snapshot (`taskId` / `status` / `loopCount` / `lastError` / `completionPath?.toUserLabel() ?: "无"` → 本地意图 / 本地 LLM / 远程 LLM / 无) + `AuditLog.listRecent`; injected `runRuleEEval` / `loadLastRuleE` for **评测意图规则 E** (`ruleEBusy` + `ruleEMessage` counts/path/`run-as` adb; last missing/empty/bad stays null and must not overwrite an in-flight or newer run); injected `runKokoroRuleB(onProgress)` / `loadLastKokoroRuleB` / `markKokoroNaturalnessOk` for **评测 Kokoro 规则 B** + **本批自然度通过** (`ruleBBusy` + download progress + `ruleBMessage`; `canMarkKokoroNaturalness` needs `nScored≥5`; Rule E and Rule B share one busy lock); still `implementation` `:core:runtime` + `:core:model` only — no `:core:tool`, ORT, or JNI |
| `:app` | `DougieApplication` builds `TaskManager` + `OpenAICompatibleProvider` + `EgressGateway` + tools + `PolicyEngine` + `HybridMemoryStore(RoomMemoryStore, AndroidEmbeddingPort)` + `DougieTaskStores` on `Dispatchers.Default`; `recoverInterrupted` + `seed`; Chat↔Settings↔OpenApps↔Memory↔Permissions↔History↔Debug routes; Chat+History `SharedTransitionLayout` in `MainActivity` (`ChatHistoryTransition.kt`; card `sharedBounds` key = `taskId`; other routes instant `when`); `LauncherApps` injects MAIN/LAUNCHER into OpenApps (no `QUERY_ALL_PACKAGES`); `DougieChatTileService` + `ChatLaunch` (Quick Settings opens Chat, no `submit`); `TaskProgressNotifier` + `formatTaskNotice` (status-only shade, tap Chat; Play attaches `BubbleMetadata` API 29+); sideload overlay via `ChannelHooks.syncOverlay` / `DougieOverlayService` (default off); Settings `shortcutLayer` slot from `ChannelHooks.ShortcutLayerSettings` (play copy = 系统气泡 only); SAF `OpenDocumentTree` + persistable permission + `ExternalModelTreeImpl` (DocumentFile, no `:core:tool`); `AppOfflineModelProbe` (ASR/TTS/intent JNI, TTS generate only, embed `AndroidEmbeddingPort.embed("测")`); `AppIntentRuleEEval` (same intent JNI seam, `filesDir/eval/intent/predictions.jsonl`, Default dispatcher; `last` is read-only `loadJsonl`/`ruleEReport`; successful `run`/`last` use `formatRuleEMessage` counts + relative path + `run-as <packageName>`); `AppKokoroRuleBEval` (eval pack under `filesDir/eval/tts/kokoro/`, not `models/tts`; `SherpaJni.generateKokoro` `numThreads=1`; `filesDir/eval/tts/kokoro-rtf.jsonl`; `formatRuleBMessage` counts + path + `run-as`; `last` missing/empty/bad → null); `ChannelHooks.seedBundledModels` (sideload copies ASR/TTS assets to `filesDir` only; play no-op — never seed embed); `ChatAttachmentSession` + Photo Picker / `TakePicture` / FileProvider camera JPEG (longest edge ≤ 1280); `CAMERA` in the main manifest is not an overlay leak |
...
**Instead**: In-app Chat avatar is three marks from `intelligenceMark(...)` in `:feature:chat`: **SUPER** (`super_dougie.xml` from `design/品牌/Super-Dougie.svg`) when a remote provider is configured and usable; **LOCAL** (`dougie_logo.xml` from `Dougie-Logo.svg`) when remote is not configured but a local **chat** LLM is ready (`ChannelHooks.localChatReady` each compose: sideload `ChatModelLayout.isPresent`, Play always false); **NOOB** (`dougie_logo_unavailable.xml` from `Noob-Dougie.svg`) when neither is configured **or** a remote call actually failed (`LLM_FAILED` / `NETWORK_FAILED` / `LLM_TIMEOUT`). Intent ONNX is not a chat LLM. Do not ship `Dougie-Logo.png` as a drawable. Adaptive **launcher** stays `dougie_logo` with 16% inset in `main` (Play and Sideload share it) — not Super or Noob.
...
**Instead**: `OfflineModelDownloads` in `:feature:settings` owns ASR ~230MB, TTS ~116MB, intent MiniRBT ONNX ~12MB int8, **语义记忆** (`models/embed/{model.onnx,vocab.txt}`, Xenova `bge-small-zh-v1.5` int8 ~24MB, catalog configured by default), and on **sideload** three chat SKUs in `models/chat/` (`chat-qwen06` / `chat-minicpm1b` / `chat-minicpm2b`, HF filenames + SHA in catalog). Play `AppOfflineModels` omits all chat rows but keeps 语义记忆. Catalog bump (new filename or SHA) in an App release: official name missing or SHA mismatch → row `needsUpdate`; **更新** sits next to **使用 / 使用中**; Chat has no update chrome. Stale still `installed` so local chat can use the old file until the new pack verifies. Confirm download writes the new official name; then delete that SKU’s other names under app-private `filesDir` / `getExternalFilesDir` `models/chat/` only — not the SAF tree, not sibling SKUs. Hash catalog files off-main keyed by path+length+mtime; do not SHA 1.5GB on compose. Unconfigured and **current** (not stale) rows do not fetch. Plus a **模型目录** row. User picks a tree via SAF; URI is persisted immediately (`PreferenceStore.setModelTreeUri`, not tied to **保存配置**). Scan lists each pack `relativeDir` under the tree, streams to temp `File`s, then `ModelImporter` syncs `filesDir` (chat import is per-SKU SHA, sibling `.litertlm` allowed). Hash-complete **current** rows show 已安装 with no HTTP. Missing → **下载** if configured and tree ready. No tree / lost persistable permission → cannot download (`请选择模型目录` / `请再次选择模型目录`). Historical `model.gguf` in the intent folder must not mark installed. `request` opens confirm for missing or stale; `confirm` calls `ModelInstaller` with `userConfirmed=true`. Installed rows show **测试**; probe is injected (`OfflineModelProbe`) and must not use system TTS as success. Cancel the job; layout `isPresent` stays false on failed hash. `:feature:settings` depends on `:core:tool` only — OkHttp, SAF, and `ContentResolver` stay in `:app` / `:tool:system`. Do not keep per-file **导入** as the primary UX.

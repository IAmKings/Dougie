...
| `:feature:history` | Task History from `TaskStore.listRecent(50)` grouped by `conversationId` (`toHistorySections`, sticky titles 「默认会话」/「对话 n」 or custom prefs name); cards show `durationLabel` / `providerLabel` / `completedAtLabel` from `toHistoryItem` (`formatTaskDuration`, `completionPath?.toUserLabel()`, `formatCompletedAt(endedAt)`); sticky **改名** dialog → `ConversationTitles.setTitle` (no `openConversation`); non-default sticky **删除** (danger color, confirm dialog) → `TaskManager.deleteConversation` then History `refresh()` plus Activity `listRecent` snapshot for Chat numbering (no `openConversation`; default has no delete); tap a card opens that window in Chat and focuses `{taskId}:user`; bottom nav **任务** |

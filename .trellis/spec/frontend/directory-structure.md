...
| `:feature:history` | Task History from `TaskStore.listRecent(50)` grouped by `conversationId` (`toHistorySections`, sticky titles 「默认会话」/「对话 n」 or custom prefs name); sticky **改名** dialog → `ConversationTitles.setTitle` (no `openConversation`); tap a card opens that window in Chat and focuses `{taskId}:user`; bottom nav **任务** |
| `:feature:debug` | Developer page: current `AgentTask` snapshot (`taskId` / `status` / `loopCount` / `lastError` / `completionPath` 本地意图 / 本地 LLM / 远程 LLM / 无) + `AuditLog.listRecent`; injected `runRuleEEval: suspend () -> String` for **评测意图规则 E** (`ruleEBusy` + `ruleEMessage` counts/path only); still `implementation` `:core:runtime` + `:core:model` only — no `:core:tool`, ORT, or JNI |

...
UI lives in `:feature:*` (Compose BOM `2024.12.01`, Material3, `lifecycle-runtime-compose`). `:app` hosts `MainActivity` routing and DI only. Product copy is **Dougie** (never Waku) and Chinese for user-visible chrome.
...
- Committing `Dougie-Logo.png` as a drawable; in-app marks are vector XML.

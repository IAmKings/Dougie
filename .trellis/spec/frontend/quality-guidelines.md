...
- [ ] Confirm Card appears only for `AWAITING_CONFIRMATION`; confirm/reject go to `TaskManager`. Composer **终止** (`canCancel`) cancels the round and is not Confirm **拒绝**; text/mic stay `inputEnabled`; TTS **停止播报** only when not busy

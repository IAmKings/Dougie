...
Verification is JVM unit tests on **pure mapping functions** (`toChatUiState`, `intelligenceMark`, `toHistoryItem`, `toHistorySections`, `formatTaskDuration`, `formatCompletedAt`, `toDebugTaskSnapshot`, `OfflineModelDownloads`, `formatTaskNotice`, `nextChatItemEnter`, `usesBubbleEnter`, `showsToolProgress`, `chatConfirmCard`, `chatFeedItemsWithoutConfirm`, `nextConfirmEnter`). Screens themselves are not tested with Compose UI tests.

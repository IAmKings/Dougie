...
Verification is JVM unit tests on **pure mapping functions** (`toChatUiState`, `intelligenceMark`, `toHistoryItem`, `toDebugTaskSnapshot`, `OfflineModelDownloads`, `formatTaskNotice`). Screens themselves are not tested with Compose UI tests.
...
- [ ] QS Tile and task-progress notice stay in `:app`, open Chat only, shade copy is status-only, Play bubbles skip sideload, overlay stays sideload-only, and `checkChannelLeak` still requires Tile + forbids NotificationListener / overlay / `QUERY_ALL_PACKAGES` / `ChatLlmSpikeActivity` on Play

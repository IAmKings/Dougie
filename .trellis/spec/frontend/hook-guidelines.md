...
- Mappers: `toChatUiState()`, `toHistoryItem()`, `formatTaskDuration()` (`:core:model`, Chat + History), `formatCompletedAt()`, `toHistorySections()`, `currentConversationTitle()`, `conversationDisplayName()`, `toDebugTaskSnapshot()`, `intelligenceMark(...)`, `nextChatItemEnter()`, `usesBubbleEnter()`, `chatConfirmCard()`, `chatFeedItemsWithoutConfirm()`, `nextConfirmEnter()`

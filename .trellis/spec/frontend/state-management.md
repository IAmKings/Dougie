...
Debug maps `TaskManager.task` to `DebugTaskSnapshot` (`taskId`, `status`, `loopCount`, `lastError`, `completionPath` as 本地意图 / 本地 LLM / 远程 LLM / 无) plus `AuditLog.listRecent` rows — never a second `TaskStatus` store, never `resultJson` / prompt / tool args. LoopEngine writes `LOCAL_LLM` when `provider.isLocal` on the LLM loop.

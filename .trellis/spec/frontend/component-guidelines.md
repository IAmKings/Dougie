...
- Wrapping `ConfirmCard` in bubble enter, or driving enter with `LazyItemScope.animateItem()` / a `firstKey` reset. Confirm has its own §11.7 motion later; `animateItem` / `firstKey` skip or replay the wrong rows.

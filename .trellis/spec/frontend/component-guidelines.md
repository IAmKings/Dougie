...
- Hardcoding tool label “电池” for every `ToolCard`. Chat maps known ids to Chinese (`battery` → 电池工具, `js_eval` → 运行脚本) and otherwise shows raw `toolName`. `confirmToolBody("js_eval")` is 隔离运行脚本，不读写文件、不上网 — do not reuse 写入设备数据.

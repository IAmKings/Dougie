...
- Chat avatar drawables: `super_dougie.xml`, `dougie_logo.xml`, `dougie_logo_unavailable.xml`. Launcher in `app/src/main/res` stays `dougie_logo` with inset — not Super/Noob. Top bar is 24sp **Dougie**, then the current window display name (12sp `OnSurfaceVariant`, ellipsis, not clickable; empty unlisted window 「新对话」), then **灵魂** (`Super` / `本地` / `不可用`), then **出境策略** (`已授权云端` / `仅本地`) so the statuses do not wrap on one row. Rename only from History **改名**.

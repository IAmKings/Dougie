...
2. `FooScreen(uiState, onEvent: ...)` — stateless UI. Local `remember { mutableStateOf }` is allowed for draft text, password visibility, dialogs, History **展开** (`remember(taskId)`), History card **删除** confirm — not for `TaskStatus`.

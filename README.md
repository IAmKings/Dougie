...
当前 **v0.1.6**（`versionCode` 106）用仓库上传密钥签名，**尚未上架任何应用商店**。这一版在 v0.1.5 之上包含设置里的常驻规则、晨间简报和「隐私」说明。若装过更早的 debug 签名包，须先卸载再装。同签名升级可覆盖安装。
...
系统要求 **Android 8.0+**（`minSdk` 26）。从 [GitHub Releases](https://github.com/IAmKings/Dougie/releases) 下载两个渠道包之一（当前 [v0.1.6](https://github.com/IAmKings/Dougie/releases/tag/v0.1.6)）：
...
- **本地优先**：云端默认关闭。顶栏能看到当前是「仅本地」还是「已授权云端」。设置页「隐私」用同一套事实说明出境、截屏、录音和密钥。
- **常驻规则**：设置顶部一段本地文本，默认关闭。打开后，云端和端侧的下一轮都把这段话接在默认身份后面。只关开关会留着正文；「恢复默认」才清空。
- **晨间简报**：定时提醒里点「晨间简报」只填入每天的草稿，仍要点「添加」。到点通知仍是「定时提醒 · 时刻」，点开填进输入框，不会自己发送。
- **权限先行**：日历、定位、麦克风、截屏等按任务申请，可在权限中心逐项关闭。
- **工具可见**：每次调用在对话里显示工具卡（默认收起，可展开）；任务页可回看完成状态。
- **记忆可控**：对话里可开关记忆门控；记住的事实在记忆页管理。
- **打开应用**：只有「可打开的应用」里的别名，用「打开」「请打开」或「帮我打开」加上别名，才会直接打开。最小档对话模型不会自己把「打开微信」当成打开应用。

侧载包还可以下载 LiteRT 对话权重（约 328MB / 756MB / 1.5GB 三档），点 **使用** 后才切换引擎；并在知情同意后使用悬浮球与无障碍手势。Play 渠道包不含这些能力，也不内置对话权重。

## 数据出境

本仓库不运营 Dougie 云。默认不上传对话。设置页「隐私」卡片写的是同一套事实。

打开出境并保存后，请求会发往你填写的 OpenAI 兼容 Base URL：用户输入、组装后的上下文（含检索到的记忆）、工具结果，以及非截屏图片附件。截屏像素留在本机，云端只看到附件说明。用麦克风说话时，录音只在本机转成文字，音频不上传。API 密钥用 AndroidX Security（`EncryptedSharedPreferences` + `MasterKey`）存在本机，不进 Logcat。

卸载应用不会删除你选中的外部模型目录；重装后需再授权一次该文件夹。

## 工具卡与分级

对话里的工具卡对应一次真实调用：默认收起，点 **展开** 可看结果（不是常驻 dump）。标题带风险等级（如「时间工具 (L0)」）。未授权的能力会被跳过，不会静默重试。用户用麦克风说的内容，气泡上会标「语音转写」。
...
1. 打开 **设置**：先看「隐私」。填写 OpenAI 兼容的 Base URL、模型与密钥。若要走云端，打开出境开关后必须点 **保存**。
2. 需要固定说法时，打开「常驻规则」并写入一段文本。默认关闭，不改变原来的身份句。
3. 对话顶栏的盾牌进入 **权限中心**，只开当前需要的能力。要让 Dougie 打开某个应用，先把它加进「可打开的应用」，再用「打开」加上别名。
4. 离线语音 / 意图：在设置里用系统文件选择器指定一个目录，约定 `{目录}/models/{asr,tts,intent}/`（识别约 230MB，合成约 116MB，意图约 12MB）。校验通过后会同步到应用私有目录再加载，不申请全盘存储。
5. 侧载对话模型：在同一设置页下载所需档位，点 **使用** 后才会作为当前引擎；下载完成不会自动切换。体积较大的档位需要更多内存，请按机型选择。最小档不会自己完成「打开某个应用」。
6. 每天看一眼日历和电量：在定时提醒里点「晨间简报」，再点「添加」。到点只发通知，点开后仍要自己发送。

...
object UserFacingErrors {
    ...
    const val LLM_EMPTY_REPLY = "模型没有给出回复，请重试。"
    const val LLM_MODEL_UNAVAILABLE = "该模型当前不可用，请更换模型后重试。"
    ...
    const val TAP_SWIPE_FOREGROUND_UNKNOWN = "无法确认当前应用，已停止点击以免误操作。"
    ...
    const val MODEL_PROBE_EMBED_OK = "语义记忆测试通过。"
    const val MODEL_PROBE_TIMEOUT = "离线模型测试超时，请稍后重试。"
    const val EMBED_MODEL_MISSING = "离线语义记忆模型尚未就绪。"
    ...
    const val PY_EVAL_TIMEOUT = "隔离 Python 运行超时，已停止。"
    const val PY_EVAL_FAILED = "隔离 Python 运行失败。"
    const val PY_ENGINE_NOT_READY = "隔离 Python 引擎尚未接入。"
